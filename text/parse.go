...
type Parser struct {
	...
	// The following only used for summaries.
...
func (p *Parser) reset(in io.Reader) {
	p.result = map[string]*dto.MetricFamily{}
	if p.buf == nil {
...
func (p *Parser) readingMetricName() stateFn {
	...
	if p.currentMF.Type == nil {
		p.currentMF.Type = dto.MetricType_CUSTOM.Enum()
	}
...
func (p *Parser) readingValue() stateFn {
	...
	case dto.MetricType_CUSTOM:
		p.currentMetric.Custom = &dto.Custom{Value: proto.Float64(value)}
...
func (p *Parser) setOrCreateCurrentMF() {
	...
	if p.currentMF = p.result[name]; p.currentMF != nil {
		return
	}
	// Try out if this is a _sum or _count for a summary.
	summaryName := summaryMetricName(name)
	if p.currentMF = p.result[summaryName]; p.currentMF != nil {
		if p.currentMF.GetType() == dto.MetricType_SUMMARY {
			if isCount(name) {
