...
//
// WARNING: It's tempting to add labels like `Client-IP` or `User Agent` that come from request headers.
// While it will give you more insights, it might belong to more logging or tracing observability.
// Metrics are optimized for stable cardinality long-term metrics. Always sanitise your label names and values to
// avoid huge monitoring costs or DoS/CVEs in your service. For example see the sanitizeCode() function.
// We also don't recommend using this function on histograms which multiples this problem dozens of times.
