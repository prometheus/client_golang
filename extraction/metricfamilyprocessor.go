...
func (m *metricFamilyProcessor) ProcessSingle(i io.Reader, out Ingester, o *ProcessOptions) error {
	...
	for {
		...
		case dto.MetricType_CUSTOM:
			if err := extractCustom(out, o, family); err != nil {
				return err
			}
		}
	}
...
func extractCustom(out Ingester, o *ProcessOptions, f *dto.MetricFamily) error {
	samples := make(model.Samples, 0, len(f.Metric))
	...
	for _, m := range f.Metric {
		...
		sample.Value = model.SampleValue(m.Custom.GetValue())
	}
