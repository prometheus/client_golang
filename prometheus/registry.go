	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, accept := range accepts {
			...
			if accept.SubType == "vnd.google.protobuf" {
				...
				switch accept.Params["proto"] {
				case "io.prometheus.client.MetricFamily":
					header.Set(contentTypeHeader, dto.MetricFamilyContentType)
					registry.dumpDelimitedPB(writer)
				case "io.prometheus.client.Sample":
					header.Set(contentTypeHeader, dto.SampleContentType)
					registry.dumpSamples(writer)
				default:
					continue
				}
